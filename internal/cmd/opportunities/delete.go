...
func runDelete(cmd *cobra.Command, args []string) error {
	...
	if !forceDelete {
		fmt.Printf("Are you sure you want to delete opportunity %s? Use --force to confirm.\n", id)
		return nil
	}

	client := rt.RESTClient()

	if err := client.DeleteOpportunity(ctx, id); err != nil {
		return fmt.Errorf("failed to delete opportunity: %w", err)
	}

	fmt.Printf("Deleted opportunity: %s\n", id)
	return nil
}
